...
class_names1 = ['Dry_skin', 'Normal_skin', 'Oily_skin']
class_names2 = ['Low', 'Moderate', 'Severe']
class_names3 = ['Low', 'Moderate', 'Severe']
skin_tone_dataset = 'models/skin_tone/skin_tone_dataset.csv'
 
def get_model():
    global model1, model2, darkCircle_model, wrinkle_model, eyebag_model, age, pore_model, redness_model
    model1 = load_model('./models/skin_model')
    print('Model 1 loaded')
    model2 = load_model('./models/acne_model')
    print("Model 2 loaded!")
    darkCircle_model = tf.keras.models.load_model('models/dark_circle_model_saved')
    print('Dark Circle model loaded!')
    eyebag_model = tf.keras.models.load_model('models/eyebags_model')
    print('Eyebag model loaded!')
    pore_model = tf.keras.models.load_model('models/pore_model_saved')
    print('Eyebag model loaded!')
    redness_model = tf.keras.models.load_model('models/redness_model_saved')
    print('Redness model loaded!')
    ...
    wrinkle_model = tf.keras.models.load_model('models/wrinkle_model_saved')  
...
def wrinkle_analysis(img_path):
    ...
    wrinkle_grade = wrinkle_model.predict(img_tensor)[0][0]
    wrinkle_grade = int(round(wrinkle_grade))
    wrinkle_grade = min(100, max(0, wrinkle_grade))
    return 100-wrinkle_grade
 
def prediction_darkCircles(img_path):
    img = cv2.imread(img_path)
    img = cv2.resize(img, (224, 224))
    img_tensor = image.img_to_array(img) / 255.0
    img_tensor = np.expand_dims(img_tensor, axis=0)
 
    darkCircle_grade = darkCircle_model.predict(img_tensor)[0][0]
    darkCircle_grade = int(round(darkCircle_grade))
    darkCircle_grade = min(100, max(0, darkCircle_grade))
    return 100-darkCircle_grade
 
def analyze_eyebags(img_path):
    img = cv2.imread(img_path)
    img = cv2.resize(img, (224, 224))
    img_tensor = image.img_to_array(img) / 255.0
    img_tensor = np.expand_dims(img_tensor, axis=0)
 
    eyebag_grade = eyebag_model.predict(img_tensor)[0][0]
    eyebag_grade = int(round(eyebag_grade))
    eyebag_grade = min(100, max(0, eyebag_grade))
    return 100-eyebag_grade
...
def redness_grade(img_path):
    ...
    redness_grade = redness_model.predict(img_tensor)[0][0]
    redness_grade = int(round(redness_grade))
    redness_grade = min(100, max(0, redness_grade))
...
#     # Filter out small/large contours that are not likely to be pores
#     pores = [contour for contour in contours if 20 < cv2.contourArea(contour) < 200]
#     # Grade based on number of pores detected
#     pore_count = len(pores)
#     pore_grade = max(0, 100 - pore_count)  # Assuming one point deducted per pore
#     return pore_grade
def pore_analysis(img_path):
    img = cv2.imread(img_path)
    img = cv2.resize(img, (128, 128))
    ...
    pore_grade = pore_model.predict(img_tensor)[0][0]
    pore_grade = int(round(pore_grade))
    pore_grade = min(100, max(0, pore_grade))
    return 100-pore_grade 
...
class SkinMetrics(Resource):
    def put(self):
        ...
        darkCircle_grade = prediction_darkCircles(file_path)
        eyebag_grade = analyze_eyebags(file_path)
        pore_grade = pore_analysis(file_path)
        age = predict_age(file_path)
        return {'type': skin_type, 'tone': str(tone), 'acne': acne_type, 'wrinkle_grade': wrinkle_grade, 'redness_grade': redness_grade_value, 'darkCircle_grade':darkCircle_grade,'eyebag_grade':eyebag_grade, 'pore_grade': pore_grade, 'age': age}, 200
...
# def get_model():
#     global model1, model2, age, wrinkle_model  
#     model1 = load_model('./models/skin_model')
#     print('Model 1 loaded')
#     model2 = load_model('./models/acne_model')
#     print("Model 2 loaded!")
...
#                 'wrinkle_grade': wrinkle_grade, 'redness_grade': redness_grade_value,
#                 'darkCircle_grade': darkCircle_grade, 'age': age,
#                 'pore_grade': pore_grade}, 200
#         # return {'type': skin_type, 'tone': str(tone), 'acne': acne_type, 'wrinkle_grade': wrinkle_grade}, 200
